...
function FusionSensor()  {
  ...
  this.listen = function (obj, feature, event, capturing, filter) {
    ...
    var r = { 'o': obj, 
              ...
              'f': feature,
              'e': 
                (!filter ? function (e) { o.trigger(feature, e) } :
                          function (e) { 
                            var evts = filter(e);
                            for (var i = 0; i < evts.length; i++) {
                              o.trigger(feature, evts[i]);
                            }})
              };
    events.push(r);
  ...
  this.trigger = function (feature, event) {
    ...
    if (!o[feature])
      o[feature] = new Event();
    triggerQueue.push({ 'feature': feature, 'event': event });
    if (triggerQueue.length == 1) {
      while (triggerQueue.length != 0) {
	var head = triggerQueue[0];
        o[head.feature].trigger(o, head);
	triggerQueue.shift()
      }
    }
  };
